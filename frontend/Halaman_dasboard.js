...
function updateFinalDecision() {
  ...
  const totalScore = projectScore * 0.55 + interviewScore * 0.45;
  let scoreLabel = "";
  if (totalScore > 90) {
...
function updateFinalRating() {
  ...
  let totalScore = projectScore * 0.55 + interviewScore * 0.45;
  let finalRating = 1;
  ...
  decisionElement.innerHTML = `
    ...
    ">
      <div class="content-text">
        Rekomendasi Perekrutan Kandidat: <b>${finalRating.toFixed(1)} (${finalRatingLabel})</b>
      </div>
    </div>
  `;
}
...
async function downloadPDF() {
  ...
  script.onload = function () {
    ...
    const llmResults = report.llmResults;
    const cheating = report.cheating;
    const nonverbal = report.nonverbal;
    ...
    doc.setFontSize(10);
    doc.text(`Session: ${report.session}`, 105, yPos, { align: "center" });
    yPos += 6;
    ...
    // ===== RINGKASAN DASHBOARD =====
    yPos = drawTableHeader(doc, "Ringkasan Dashboard", yPos, tableX, blue);
    const summaryData = [
      ["Confidence Score", `${llmResults.avg_logprobs_confidence}%`],
      ["Rating", `${report.finalRating}/5 (${report.finalRatingLabel})`],
    ];
    yPos = drawTable(
      doc,
      summaryData,
    ...
    doc.setFontSize(10);
    doc.setTextColor(...blue);
    doc.text("Final Decision:", 20, yPos);
    yPos += 5;
    doc.setFontSize(9);
    ...
    yPos += decisionLines.length * 5 + 5;

    if (llmResults.kesimpulan_llm) {
      doc.setTextColor(...blue);
      doc.text("Kesimpulan LLM:", 20, yPos);
      yPos += 5;
      doc.setTextColor(0, 0, 0);
      const conclusionLines = doc.splitTextToSize(
        llmResults.kesimpulan_llm,
        170,
      );
    ...
    // ===== RINGKASAN DETEKSI =====
    yPos = drawTableHeader(doc, "Ringkasan Deteksi", yPos + 5, tableX, blue);
    const detectionSummary = [
      ["Final Verdict", cheating.final_aggregate_verdict],
      ["Risk Level", cheating.risk_level],
      ["Cheating Score", `${cheating.avg_cheating_score}%`],
      ["Overall Confidence", `${cheating.avg_overall_confidence.toFixed(2)}%`],
      [
        "Non-Verbal Confidence",
        `${nonverbal.overall_confidence_score.toFixed(2)}%`,
      ],
      [
        "Transcription Confidence",
        `${report.transcriptionConfidence.toFixed(2)}%`,
      ],
    ];
    yPos = drawTable(
      doc,
    ...
    yPos = drawTableHeader(doc, "Confidence Cards", yPos, tableX, blue);
    const confidenceData = [
      ["Cheating Detection", `${cheating.avg_overall_confidence.toFixed(2)}%`],
      [
        "Non-Verbal Analysis",
        `${nonverbal.overall_confidence_score.toFixed(2)}%`,
      ],
      ["Transcription", `${report.transcriptionConfidence.toFixed(2)}%`],
    ];
    yPos = drawTable(
      doc,
      confidenceData,
...
function buildDashboardReportData() {
  ...
  const content = Array.isArray(interviewData.content)
  ...
  const transcriptionConfidence = content.length
    ? content.reduce(
        (sum, item) => sum + Number(item.result?.transkripsi_confidence || 0),
        0,
      ) / content.length
    : 0;
  ...
  return {
    ...
    llmResults: {
      avg_logprobs_confidence: Number(llmResults.avg_logprobs_confidence || 0),
      kesimpulan_llm: llmResults.kesimpulan_llm || "",
      avg_total_llm: Number(llmResults.avg_total_llm || 0),
    },
    cheating: {
      final_aggregate_verdict: cheating.final_aggregate_verdict || "-",
      risk_level: cheating.risk_level || "-",
      avg_cheating_score: Number(cheating.avg_cheating_score || 0),
      avg_visual_confidence: Number(cheating.avg_visual_confidence || 0),
      avg_audio_confidence: Number(cheating.avg_audio_confidence || 0),
      avg_overall_confidence: Number(cheating.avg_overall_confidence || 0),
      total_suspicious_frames: Number(cheating.total_suspicious_frames || 0),
    },
    nonverbal: {
      overall_performance_status: nonverbal.overall_performance_status || "N/A",
      overall_confidence_score: Number(nonverbal.overall_confidence_score || 0),
...
function getRatingCategoryText(score) {
